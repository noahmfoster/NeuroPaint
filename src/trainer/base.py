...
class Trainer():
    def __init__(
        ...
        self.target_kernel_stored = {frozenset({areaoi_ind[i], areaoi_ind[j]}): {'kernel':torch.ones(num_factors, num_factors).to(self.accelerator.device), 'count':0} for i in range(num_areas) for j in range(i, num_areas)}
    ...
    def _update_target_kernel(self, batch):
        ...
        x = self.encoder_stitcher_ema(batch['spikes_data'], str(batch['eid'][0].item()), batch['neuron_regions'])
        ...
        for i in range(len(area_ind_unique)):
            ...
            for j in range(i, len(area_ind_unique)):
                ...
                x_i = x[:,:,i,:].view(-1, x.shape[-1]) # (B*T, n_channels_per_region)
                x_j = x[:,:,j,:].view(-1, x.shape[-1]) 

                kernel = torch_corrcoef(x_i, x_j) # (n_channels_per_region, n_channels_per_region)
                
                key_tmp = frozenset({area_i, area_j})
                stored_kernel = self.target_kernel_stored[key_tmp]['kernel']
                count = self.target_kernel_stored[key_tmp]['count']
                self.target_kernel_stored[key_tmp]['kernel'] = (stored_kernel * count + kernel) / (count + 1)
                self.target_kernel_stored[key_tmp]['count'] += 1
        
    def _compute_consistency_loss(self, batch):
        model_encoder = self.model.module.encoder if self.multi_gpu else self.model.encoder
        x = model_encoder.stitchers(batch['spikes_data'], str(batch['eid'][0].item()), batch['neuron_regions'])
        area_ind_unique = batch['neuron_regions'][0].unique()
        ...
        for i in range(len(area_ind_unique)):
            ...
            for j in range(i, len(area_ind_unique)):
                area_j = area_ind_unique[j].item()
                x_i = x[:,:,i,:].view(-1, x.shape[-1])
                x_j = x[:,:,j,:].view(-1, x.shape[-1])

                kernel = torch_corrcoef(x_i, x_j)
                key_tmp = frozenset({area_i, area_j})
                target_kernel = self.target_kernel_stored[key_tmp]['kernel']
                if j==i:
