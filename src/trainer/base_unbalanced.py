...
class Trainer():
    def __init__(
        ...
        self.target_kernel_stored = {'hit_left': target_kernel_each_type, 'hit_right': target_kernel_each_type} 
    ...
    def train_epoch(self):
        ...
        if self.consistency:
            return{
                ...
                "train_consistency_loss": (train_consistency_loss.item() if isinstance(train_consistency_loss, torch.Tensor) else train_consistency_loss)/len(self.train_dataloader),
    ...
    def _update_target_kernel(self, batch):
        ...
        for i in range(len(area_ind_unique)):
            ...
            for j in range(i, len(area_ind_unique)):
                ...
                for trial_type in ['hit_left', 'hit_right']:

                    if trial_type == 'hit_left':
                        trial_type_flag = (batch['trial_type'] == 0)
                    elif trial_type == 'hit_right':
                        trial_type_flag = (batch['trial_type'] == 21)
    ...
    def eval_epoch(self): 
        ...
        if self.consistency:
            return {
                ...
                "eval_consistency_loss": (eval_consistency_loss.item() if isinstance(eval_consistency_loss, torch.Tensor) else eval_consistency_loss) / len(self.eval_dataloader),
