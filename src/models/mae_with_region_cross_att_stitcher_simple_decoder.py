...
class NeuralStitcher_cross_att(nn.Module):
    def __init__(self, 
        ...
        self.key = nn.Linear(config.T + config.unit_embed_dim + config.region_embed_dim, config.hidden_size, bias=False)
        self.value = nn.Linear(config.T + config.unit_embed_dim + config.region_embed_dim, config.hidden_size, bias=False)
        self.n_channels_per_region = config.n_channels_per_region
        self.cross_att_dropout = nn.Dropout(config.dropout_cross_att)

        self.ln1 = nn.LayerNorm(config.T + config.unit_embed_dim + config.region_embed_dim)
    ...
    def forward(self, x, eid, neuron_regions):
        ...
        x_concat = torch.cat([x_transpose, unit_vec, region_vec], dim=2) # (B, N, T+D1+D2)
...
class StitchDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        for area_ind_list, session_ind in zip(area_ind_list_list, session_list):
            ...
            for area_ind in area_ind_unique:
                ...
                stitch_decoder_dict[str(session_ind)][str(area_ind.item())] = nn.Linear(n_channels_per_region, len(neuron_indices))
        ...
        self.Linear = nn.Linear(n_channels, n_channels_per_region)
    ...
    def forward(self, x, eid, neuron_regions):
        ...
        for area_ind in area_ind_unique:
            area_index = torch.where(self.areaoi_ind == area_ind)[0]
            x2 = stitch_decoder[str(area_ind.item())](x1[:,:, area_index, :])
...
class NeuralEncoder(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Mask layer
        self.H1 = self.hidden_size-config.region_embed_dim 
    ...
    def forward(
            ...
            neuron_regions:   Optional[np.ndarray] = None,  # (B, N)
        ...
        # stitcher
        x = self.stitchers(spikes, str(eid.item()), neuron_regions) # (B, T, R, n_channels)
        ...
        #area_ind_unique_ind = torch.nonzero(torch.isin(self.areaoi_ind, area_ind_unique)).squeeze()
...
class MAE_with_region_stitcher(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.stitch_decoder = StitchDecoder(kwargs['eids'], 
                                            ...
                                            config.decoder.stitcher.n_channels_per_region, 
    ...
    def forward(
        ...
        x, mask, mask_R, mask_T, ids_restore_R, ids_restore_T = self.encoder(spikes, spikes_timestamps, area_ind_unique, neuron_regions, trial_type, masking_mode, eid, force_mask)
        #mask is (B, T, R, H)
        #x is (B, T, R_all, H)
